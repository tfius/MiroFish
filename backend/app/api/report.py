...
def download_report(report_id: str):
    ...
    try:
        ...
        if not os.path.exists(md_path):
            # If the Markdown file does not exist, generate a temporary one
            import tempfile
...
def check_report_status(simulation_id: str):
    ...
    try:
        ...
        # The interview feature is only unlocked once the report is complete
...
def get_agent_log(report_id: str):
    ...
    Retrieves each action taken during report generation in real time, including:
    - Report start, planning start/complete
    - Each section's start, tool calls, LLM responses, and completion
    - Report completion or failure

    Query parameters:
        from_line: line number to start reading from (optional, default 0, for incremental fetching)
...
def get_console_log(report_id: str):
    ...
    Retrieves console output (INFO, WARNING, etc.) from the report generation process in real time.
    Unlike the agent-log endpoint which returns structured JSON, this returns plain-text
    console-style log lines.
