...
Fully automated — no manual parameter configuration required.

Employs a step-by-step generation strategy to avoid failures caused by generating
too much content in a single call:
...
CHINA_TIMEZONE_CONFIG = {
    ...
    "activity_multipliers": {
        ...
        "night": 0.5       # declining in the late night
    }
}
...
class AgentActivityConfig:
    ...
    # Influence weight (determines probability that other agents see this agent's posts)
...
class EventConfig:
    """Event configuration"""
    # Initial events (events triggered at the start of simulation)
...
class PlatformConfig:
    ...
    # Viral spread threshold (number of interactions before triggering viral spread)
...
class SimulationConfigGenerator:
    ...
    Uses an LLM to analyse simulation requirements, document content, and graph entity
...
- The user group is Chinese; the configuration must conform to Beijing Time daily schedule habits
...
- General pattern: low in the early hours, gradually increasing in the morning, moderate during work hours, peak in the evening
- **Important**: the example values below are for reference only; adjust the specific time slots based on the nature of the event and the characteristics of the participant groups
  - E.g., students peak at 21:00–23:00; media are active all day; official institutions are only active during work hours
  - E.g., a sudden trending event may cause discussion late at night, so off_peak_hours can be shortened accordingly
...
- peak_hours (int array): peak hours, adjusted based on the event's participant groups
- off_peak_hours (int array): off-peak hours, usually the early morning
...
- reasoning (string): brief explanation of why this configuration was chosen"""
        ...
        system_prompt = "You are a social media simulation expert. Return pure JSON; the time configuration must conform to Chinese daily schedule habits."
    ...
    def _get_default_time_config(self, num_entities: int) -> Dict[str, Any]:
        """Get the default time configuration (Chinese daily schedule)"""
        return {
            "total_simulation_hours": 72,
    ...
    def _parse_time_config(self, result: Dict[str, Any], num_entities: int) -> TimeSimulationConfig:
        """Parse the time configuration result and validate that agents_per_hour values do not exceed total agent count"""
        # Get raw values
        ...
        if agents_per_hour_min > num_entities:
            logger.warning(f"agents_per_hour_min ({agents_per_hour_min}) exceeds total agent count ({num_entities}), correcting")
            agents_per_hour_min = max(1, num_entities // 10)

        if agents_per_hour_max > num_entities:
            logger.warning(f"agents_per_hour_max ({agents_per_hour_max}) exceeds total agent count ({num_entities}), correcting")
        ...
        return TimeSimulationConfig(
            ...
            off_peak_activity_multiplier=0.05,  # almost no one in the early hours
    ...
    def _generate_event_config(
        ...
        # Get a list of available entity types for the LLM to reference
...
For example: official announcements should be published by an Official/University type; news by MediaOutlet; student opinions by Student.
    ...
    def _assign_initial_post_agents(
        ...
        for post in event_config.initial_posts:
            ...
            if matched_agent_id is None:
                ...
                if agent_configs:
                    # Sort by influence weight and pick the highest
...
- **Schedule must reflect Chinese daily habits**: almost no activity 00:00–05:00, most active in the evening 19:00–22:00
...
{{
    "agent_configs": [
        {{
            ...
            "active_hours": [<list of active hours following Chinese daily schedule>],
        ...
        system_prompt = "You are a social media behaviour analysis expert. Return pure JSON; configurations must conform to Chinese daily schedule habits."
        ...
        for i, entity in enumerate(entities):
            ...
            # If the LLM did not produce a config, use rule-based generation
            if not cfg:
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        """Generate a single Agent configuration using rules (Chinese daily schedule)"""
