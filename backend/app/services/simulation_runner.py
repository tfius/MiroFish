...
class SimulationRunState:
    ...
    # Per-platform independent round and simulated-time counters (for dual-platform parallel display)
    ...
    # Platform completion flags (detected via simulation_end event in actions.jsonl)
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            # Per-platform independent round and time counters
...
class SimulationRunner:
    ...
    # Directory for storing run states
    ...
    def start_simulation(
        ...
        try:
            ...
            # If max_rounds is specified, append it as a CLI argument
            if max_rounds is not None and max_rounds > 0:
                cmd.extend(["--max-rounds", str(max_rounds)])

            # Create main log file to avoid blocking the process due to a full stdout/stderr pipe buffer
            main_log_path = os.path.join(sim_dir, "simulation.log")
            main_log_file = open(main_log_path, 'w', encoding='utf-8')

            # Set subprocess environment variables to ensure UTF-8 encoding on Windows.
            # This fixes issues where third-party libraries (e.g. OASIS) open files without
            ...
            # Use start_new_session=True to create a new process group so all child processes can be
            # terminated via os.killpg when the server shuts down.
            process = subprocess.Popen(
                ...
                start_new_session=True,  # Create new process group so all related processes can be terminated on server shutdown
            )

            # Save file handles for later closing
            cls._stdout_files[simulation_id] = main_log_file
            cls._stderr_files[simulation_id] = None  # Separate stderr handle no longer needed
    ...
    def _monitor_simulation(cls, simulation_id: str):
        ...
        try:
            ...
            # After process exits, do a final log read
            if os.path.exists(twitter_actions_log):
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                # Update round info from round_end event
                                    ...
                                    # Update per-platform independent round and time counters
                                    if platform == "twitter":
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        ...
        if IS_WINDOWS:
            ...
            # /F = force terminate, /T = terminate the process tree (including children)
            ...
            try:
                ...
                # If the process group has not exited after the timeout, force-send SIGKILL
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Resolve platform: prefer the record's own platform field, fall back to the default
    ...
    def get_all_actions(
        ...
        # If per-platform files do not exist, fall back to the legacy single-file format
        if not actions:
            actions_log = os.path.join(sim_dir, "actions.jsonl")
            actions = cls._read_actions_from_file(
                actions_log,
                default_platform=None,  # The legacy format file should have a platform field
                platform_filter=platform,
                agent_id=agent_id,
                round_num=round_num
            )
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Clean up the simulation run logs (used to force a fresh restart of the simulation).
    ...
    def cleanup_all_simulations(cls):
        ...
        # Check whether there is anything to clean up (avoid printing useless logs when there are no processes)
        ...
        # Stop all graph memory updaters first (stop_all prints its own logs)
        try:
        ...
        # Copy the dict to avoid modifying it while iterating
            ...
            try:
                if process.poll() is None:  # Process is still running
                    ...
                    try:
                        ...
                        # Process may already be gone; try direct termination
                        try:
                            process.terminate()
                    ...
                    # Also update state.json to reflect the stopped status
                    try:
                        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
    ...
    def register_cleanup(cls):
        ...
        Called at Flask application startup to ensure all simulation processes are cleaned up
        ...
        # In non-debug mode this environment variable is not set, but we still need to register.
        is_reloader_process = os.environ.get('WERKZEUG_RUN_MAIN') == 'true'
        is_debug_mode = os.environ.get('FLASK_DEBUG') == '1' or os.environ.get('WERKZEUG_RUN_MAIN') is not None

        # In debug mode, only register in the reloader child; in non-debug mode always register.
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Target platform (optional)
                ...
                - None: In a dual-platform simulation, interview both platforms and return a merged result
        ...
        Raises:
            ValueError: Simulation not found or environment is not running
        ...
        if not ipc_client.check_env_alive():
            raise ValueError(f"Simulation environment is not running or has shut down; cannot perform interview: {simulation_id}")
    ...
    def interview_agents_batch(
        ...
        Args:
            ...
            platform: Default platform (optional; overridden by the platform field of each interview item)
                ...
                - None: In a dual-platform simulation, interview both platforms for each agent
            timeout: Timeout in seconds
    ...
    def interview_all_agents(
        ...
        Args:
            simulation_id: Simulation ID
            prompt: Interview question (used for all agents)
            platform: Target platform (optional)
                - "twitter": Interview only the Twitter platform
                - "reddit": Interview only the Reddit platform
                - None: In a dual-platform simulation, interview both platforms for each agent
    ...
    def close_simulation_env(
        ...
        Sends a close-environment command to the simulation, causing it to exit the
    ...
    def _get_interview_history_from_db(
        ...
        """Fetch interview history from a single database"""
        import sqlite3
    ...
    def get_interview_history(
        ...
        """
        Get interview history (read from the database).

        Args:
            simulation_id: Simulation ID
            platform: Platform type (reddit/twitter/None)
                - "reddit": Only fetch Reddit platform history
                - "twitter": Only fetch Twitter platform history
                - None: Fetch history from both platforms
            agent_id: Specific agent ID (optional; only fetch history for that agent)
        ...
        if platform in ("reddit", "twitter"):
            ...
            # When no platform is specified, query both
