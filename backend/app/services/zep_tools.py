...
class InsightForgeResult:
    ...
    Contains retrieval results for multiple sub-questions and a consolidated analysis
    """
    query: str
...
class PanoramaResult:
    ...
    def to_text(self) -> str:
        ...
        # Current active facts (complete output, no truncation)
        if self.active_facts:
            text_parts.append(f"\n### [Current Active Facts] (simulation results verbatim)")
    ...
    def to_text(self) -> str:
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                # Filter out content containing question numbers (问题1-9)
                skip = False
                for d in '123456789':
...
class InterviewResult:
    ...
    def to_text(self) -> str:
        ...
        text_parts = [
            ...
            f"**Interviews:** {self.interviewed_count} / {self.total_agents} simulated agents",
...
class ZepToolsService:
    ...
    [Core retrieval tools - optimized]
    1. insight_forge - deep insight retrieval (most powerful; auto-generates sub-questions, multi-dimension retrieval)
    ...
    - get_all_nodes - get all nodes from a graph
    - get_all_edges - get all edges from a graph (with temporal info)
    ...
    def get_all_edges(self, graph_id: str, include_temporal: bool = True) -> List[EdgeInfo]:
        """
        Get all edges from a graph (with pagination, including temporal information)
    ...
    def get_node_edges(self, graph_id: str, node_uuid: str) -> List[EdgeInfo]:
        ...
        Retrieves all graph edges and filters for those connected to the specified node
    ...
    def get_simulation_context(
        ...
        Searches comprehensively for all information related to the simulation requirement

        Args:
            graph_id: Graph ID
            simulation_requirement: Simulation requirement description
            limit: Limit on the number of items of each type
        ...
        # Filter entities that have an actual type (not pure Entity nodes)
        entities = []
        for node in all_nodes:
    ...
    def insight_forge(
        ...
        The most powerful hybrid retrieval function; automatically decomposes the question and searches across multiple dimensions:
        ...
        Args:
            ...
            query: User question
            simulation_requirement: Simulation requirement description
            report_context: Report context (optional; used for more precise sub-question generation)
        ...
        entity_insights = []
        node_map = {}  # Used later for relationship chain construction
...
Return a JSON-formatted list of sub-questions."""
    ...
    def panorama_search(
        ...
        Gets a full-picture view, including all related content and historical/expired information:
        1. Retrieves all related nodes
        2. Retrieves all edges (including expired/invalidated ones)
        3. Categorizes and organizes current active and historical information

        This tool is suited for scenarios that require understanding the complete picture of an event or tracing its evolution.
    ...
    def interview_agents(
        ...
        2. Uses LLM to analyze the interview requirement and intelligently selects the most relevant agents
        ...
        [Use cases]
        - Need to understand different stakeholders' views on an event from multiple role perspectives
        - Need to collect opinions and viewpoints from multiple parties
        - Need real answers from simulated agents (not LLM simulation)
        ...
        if not profiles:
            ...
            result.summary = "No interviewable agent persona file found"
            return result
        ...
        # Add an optimizing prefix to constrain the agent's reply format
        INTERVIEW_PROMPT_PREFIX = (
            ...
            "4. Answer each question by number; begin each answer with 'Question X:' (X is the question number)\n"
        ...
        try:
            # Build the batch interview list (no platform specified; interview on both platforms)
            interviews_request = []
            for agent_idx in selected_indices:
                interviews_request.append({
                    "agent_id": agent_idx,
                    "prompt": optimized_prompt  # Use the optimized prompt
                    # No platform specified; the API interviews on both twitter and reddit
                })
            ...
            api_result = SimulationRunner.interview_agents_batch(
                ...
                timeout=180.0   # Longer timeout needed for both platforms
            )
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                # Collect interview results for this agent from both platforms
                ...
                # Strip any tool-call JSON wrappers from the raw response
                twitter_response = self._clean_tool_call_response(twitter_response)
                reddit_response = self._clean_tool_call_response(reddit_response)

                # Always output dual-platform markers
                twitter_text = twitter_response if twitter_response else "(no reply received)"
                reddit_text = reddit_response if reddit_response else "(no reply received)"
    ...
    def _clean_tool_call_response(response: str) -> str:
        """Strip JSON tool-call wrappers from agent replies and extract actual content"""
    ...
    def _select_agents_for_interview(
        ...
        system_prompt = """You are a professional interview planning expert. Your task is to select the most suitable interview subjects from the list of simulated agents based on the interview requirements.

Selection criteria:
1. The agent's identity/profession is relevant to the interview topic
2. The agent may hold unique or valuable perspectives
3. Choose a diverse range of viewpoints (e.g. supporters, opponents, neutral parties, professionals, etc.)
...
2. Questions that may have different answers depending on the respondent's role
...
- You may use **bold** to highlight key terms, but avoid other Markdown syntax"""
