...
from ..local_zep import Zep, EpisodeData, EntityEdgeSourceTarget
from ..local_zep.ontology import EntityModel, EntityText, EdgeModel
...
class GraphBuilderService:
    ...
    def get_graph_data(self, graph_id: str) -> Dict[str, Any]:
        """
        Retrieve the complete graph data (including detailed information)

        Args:
            graph_id: Graph ID

        Returns:
            A dict containing nodes and edges with timestamps, attributes, and other details
        """
        nodes = fetch_all_nodes(self.client, graph_id)
        edges = fetch_all_edges(self.client, graph_id)

        # Create a node mapping to look up node names
        node_map = {}
        for node in nodes:
