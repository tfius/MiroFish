...
This module wraps the automatic pagination logic (including per-page retry)
and returns the complete list transparently to callers.
"""
...
from ..local_zep import InternalServerError, Zep
...
def _fetch_page_with_retry(
    ...
    **kwargs: Any,
    ...
    """Single-page request with exponential-backoff retry on failure. Retries only transient network/IO errors."""
    if max_retries < 1:
...
def fetch_all_nodes(
    ...
    """Fetch graph nodes with pagination, returning at most max_items (default 2000). Each page request includes retry."""
    all_nodes: list[Any] = []
