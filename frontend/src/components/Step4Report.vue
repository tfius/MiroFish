            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content ready, but the full section may not be complete yet) -->
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- When this is the final answer, show a special hint -->
...
const toggleRawResult = (timestamp, event) => {
  ...
  // After the DOM updates, adjust the scroll position to keep the button in the same place
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract statistics - matching the "相关预测事实: X条" format
    ...
    const entitySection = text.match(/### \[Key Entities\]\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
...
const parseInterview = (text) => {
  ...
  try {
    ...
    const countMatch = text.match(/\*\*Interviews:\*\*\s*(\d+)\s*\/\s*(\d+)/)
    if (countMatch) {
    ...
    // Extract selection reason for interview subjects
    const reasonMatch = text.match(/### Subject Selection Rationale\n([\s\S]*?)(?=\n---\n|\n### Interview Records)/)
    if (reasonMatch) {
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // Extract title (e.g. "Student", "Education Worker")
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // If the first question is prefixed with "1.", special handling is needed
      ...
      // Extract answers - separated by Twitter and Reddit
      ...
      if (answerMatch) {
        ...
        // Platform fallback logic (legacy format: only one platform marker present)
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer: copy as default display only if not placeholder text
          if (interview.redditAnswer && interview.redditAnswer !== '(no reply received)') {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch && !redditMatch) {
          if (interview.twitterAnswer && interview.twitterAnswer !== '(no reply received)') {
            interview.redditAnswer = interview.twitterAnswer
          }
        } else if (!twitterMatch && !redditMatch) {
          // No platform markers at all (very old format): use entire text as answer
      ...
      if (quotesMatch) {
        ...
        // Prefer matching the > "text" format
            ...
            (expandedHistorical.value ? props.result.historicalFacts : props.result.historicalFacts.slice(0, INITIAL_SHOW_COUNT)).map((fact, i) => 
              h('div', { class: 'fact-item historical', key: i }, [
                ...
                h('div', { class: 'fact-content' }, [
                  // Attempt to extract time info [time - time]
                  (() => {
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Check whether this is a platform placeholder text
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '(no reply received)' || t === '（该平台未获得回复）' || t === '(该平台未获得回复)' || t === '[无回复]'
    }
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // Prefer matching the "问题X：" format first
      const cnPattern = /(?:^|[\r\n]+)问题(\d+)[：:]\s*/g
      ...
      // If no numbers found, or only one found, return the whole text
      if (matches.length <= 1) {
    ...
    // Check whether a given question has answers from both platforms (filtering out placeholder text)
...
// The currently active step (shown in the header)
...
const renderMarkdown = (content) => {
  ...
  // Strip leading second-level heading (## xxx) because the section title is already shown in the outer layer
  ...
  // Strip whitespace after list opening tags
  html = html.replace(/<ul class="md-ul">\s+/g, '<ul class="md-ul">')
  html = html.replace(/<ol class="md-ol">\s+/g, '<ol class="md-ol">')
  // Strip whitespace before list closing tags
  ...
  // Clean up <br> immediately after block-level closing tags, before paragraph opening
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix numbering for non-consecutive ordered lists: keep counter incrementing when single-item <ol>s are separated by paragraph content
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'section_complete') {
            if (log.details?.content) {
              ...
              // Auto-expand the freshly generated section
          ...
          if (log.action === 'report_complete') {
            ...
            stopPolling()
            // Scroll logic is handled together in nextTick after the loop ends
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // If the task is complete, scroll to the top; otherwise scroll to the bottom to follow the latest logs
            if (isComplete.value) {
...
// Extract the final answer content - extracts section content from the LLM response
const extractFinalContent = (response) => {
  ...
  // Try to find the content after "Final Answer:" (supports multiple formats)
  ...
  // Try to find the content after "最终答案:"
  const chineseFinalMatch = response.match(/最终答案[:：]\s*\n*([\s\S]*)$/i)
  if (chineseFinalMatch) {
    return chineseFinalMatch[1].trim()
  }
  
  // If the content starts with ## or # or >, it may be direct markdown content
  const trimmedResponse = response.trim()
  if (trimmedResponse.match(/^[#>]/)) {
    return trimmedResponse
  }
  
  // If the content is long and contains markdown formatting, try removing the thinking process before returning
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Remove the "Thought:" thinking process at the beginning
