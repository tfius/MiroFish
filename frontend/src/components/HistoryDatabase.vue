...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate cards in the current row to ensure each row is centered
    ...
    // Expand downward, adding spacing from the title
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update the pending target state (always record the latest target, even during animation)
        ...
        // If animating, only record the state and handle it after the animation ends
        if (isAnimating) return

        // If the target state is the same as the current state, nothing to do
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check whether the pending state still needs to execute (may have been overridden by later scroll)
          ...
          setTimeout(() => {
            ...
            // After animation ends, check if there is a new pending state to execute
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Delay a short time before executing to avoid switching too quickly
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    },
    {
      ...
      // Adjust rootMargin: shrink viewport bottom upward so more scrolling is needed to trigger expand
...
// Watch for route changes and reload data when returning to the home page
...
onMounted(async () => {
  // Ensure DOM has rendered before loading data
  await nextTick()
  await loadHistory()

  // Initialize observer after DOM has rendered
  setTimeout(() => {
    initObserver()
  }, 100)
})
...
.grid-pattern {
  ...
  /* Anchor at top-left so height changes only extend from the bottom, keeping existing grid position stable */
