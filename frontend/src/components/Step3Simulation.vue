...
// Reset all state (used for restarting the simulation)
const resetAllState = () => {
  phase.value = 0
  runStatus.value = {}
  allActions.value = []
...
// Track the previous round for each platform to detect changes and emit logs
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect whether the simulation has completed (via runner_status or platform completion state)
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      // Use all_actions to obtain the complete action list
      ...
      // Do not auto-scroll; let the user freely navigate the timeline
