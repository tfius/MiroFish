...
// Dismiss the simulation-ended hint
...
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Transitioned from simulating to not simulating — show the ended hint
...
const renderGraph = () => {
  ...
  // Process edge data: count and index edges between the same pair of nodes
  const edgePairCount = {}
  const selfLoopEdges = {} // Self-loop edges grouped by node
  const tempEdges = edgesData
    .filter(e => nodeIds.has(e.source_node_uuid) && nodeIds.has(e.target_node_uuid))
  
  // Count the number of edges between each pair of nodes and collect self-loop edges
  tempEdges.forEach(e => {
    if (e.source_node_uuid === e.target_node_uuid) {
      // Self-loop — collect into an array
  ...
  // Track the index of the edge currently being processed for each node pair
  const edgePairIndex = {}
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      ...
      edges.push({
        ...
        rawData: {
          ...
          selfLoopEdges: allSelfLoops // Store detailed information for all self-loop edges
        }
      })
      return
    }
    ...
    // Determine whether the edge direction matches the normalized direction (source UUID < target UUID)
    const isReversed = e.source_node_uuid > e.target_node_uuid

    // Calculate curvature: spread multiple edges apart, single edge is a straight line
    let curvature = 0
    if (totalCount > 1) {
      // Evenly distribute curvature to ensure clear visual separation
      // Curvature range increases with edge count — more edges means a wider range
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2

      // Flip curvature if the edge direction is opposite to the normalized direction,
      // so all edges spread within the same reference frame and don't overlap
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // Dynamically adjust distance based on the number of edges between this node pair
      // Base distance 150, plus 50 per additional edge
    ...
    // Add gravity toward the center so isolated node clusters gather near the center
  ...
  const getLinkPath = (d) => {
    ...
    if (d.isSelfLoop) {
      ...
      const loopRadius = 30
      // Depart from the right side of the node and loop back
    ...
    // Perpendicular offset to the line direction, computed as a proportion of distance to ensure the curve is clearly visible
  ...
  // Compute curve midpoint (used for label positioning)
  const getLinkMidpoint = (d) => {
    ...
    // Midpoint of a quadratic Bézier curve at t=0.5
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    const pairTotal = d.pairTotal || 1
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        ...
        if (!d._isDragging && distance > 3) {
          // Only restart simulation when a true drag is detected for the first time
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Only let the simulation cool down if the node was actually dragged
        if (d._isDragging) {
...
/* Style for the hint shown after simulation ends */
