            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  After uploading documents, the LLM analyzes the content and automatically generates an ontology structure suitable for public opinion simulation (entity types + relation types).
                </div>
              </div>
...
const goToNextStep = () => {
  // TODO: Proceed to the environment setup step
...
// Handle new project - calls the ontology/generate API
...
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      // Resume polling for an in-progress build task
...
const startGraphPolling = () => {
  ...
  // Then automatically fetch graph data every 10 seconds
  ...
  try {
    ...
    if (projectResponse.success && projectResponse.data.graph_id) {
      ...
      if (graphResponse.success && graphResponse.data) {
        ...
        // Re-render when data has changed
...
const renderGraph = () => {
  ...
  // Draw edges (with a wide transparent clickable overlay)
  ...
  // Wide transparent line for click detection
  ...
  simulation.on('tick', () => {
    // Update position of all edges (both visible and transparent click areas)
